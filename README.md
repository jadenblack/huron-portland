...
Or manually: `cd docs/presentation && npm install && npm run dev`
...
│   ├── presentation/            # Interactive animated slideshow (Vite + Three.js)
│   └── workflow-engine/         # Workflow Engine specifications
└── src/
    ├── backend/                 # Golang backend (Clean Architecture)
    ├── dashboard/               # Next.js 14 dashboard (HeroUI + Three.js)
    └── frontend/                # React/Next.js frontend
```

### Dashboard (`src/dashboard/`)

A standalone Next.js 14 dashboard for visualizing grant portfolio data. Runs independently on port 3002 with zero backend dependencies (mock data mode).
...
|------|-------------|--------------|
| `/` | KPI cards, status overview, recent proposals | HeroUI Cards, TanStack Query |
| `/proposals` | Card and table views with filtering | HeroUI Table, Tabs, Chips |
| `/analytics` | Interactive 3D charts (status, budget, timeline) | Three.js, React Three Fiber |
| `/search` | Semantic vector search with similarity scores | rvlite WASM, cosine similarity |

**Stack:** Next.js 14 App Router, HeroUI component library, Three.js 3D visualizations, rvlite WASM vector search, TanStack Query, Tailwind CSS, Framer Motion, next-themes (dark/light mode).

See [`src/dashboard/README.md`](src/dashboard/README.md) and [`src/dashboard/adrs/`](src/dashboard/adrs/) for architecture decisions.
...
# Opens at http://localhost:3003/presentation/
```

| Feature | Description |
|---------|-------------|
| **Three.js Backgrounds** | Particle fields, geometric shapes, flowing lines, neural networks, circuit boards |
| **Slide Navigation** | Arrow keys, Space, swipe gestures, clickable side panel |
| **Fullscreen Mode** | Press F to enter 16:9 fullscreen presentation |
| **Side Panel** | Press T to toggle slide list drawer for quick navigation |
| **PDF Export** | Download all slides as a landscape PDF |
| **Framer Motion** | Fade, slide, and zoom transitions between slides |
| **Code Highlighting** | Syntax-highlighted code blocks for Go, TypeScript, SQL, Bash |

**Stack:** Vite, React 18, TypeScript, Three.js (React Three Fiber + Drei), Framer Motion, Tailwind CSS, html2canvas + jsPDF, PrismJS.

**Slides:** Welcome, Grants System Overview, What is Agentic Engineering, Toolchain, Setup, ADRs & DDD, Repository Structure, Database & Auth, RuVector Deep Dive, Backend Go, Frontend Next.js, Self-Learning, Live Build Plan, Testing & Validation, Wrap-Up.
